...
async def list_items_route(
    ...
    skip = (page - 1) * limit
    items, total = await list_items(db, user_id=x_user_id, skip=skip, limit=limit)
...
async def create_item_route(
    ...
    item = await create_item(db, user_id=x_user_id, data=data)
...
async def get_item_route(
    ...
    item = await get_item(db, item_id=item_id, user_id=x_user_id)
    return ItemResponse.model_validate(item)
...
async def update_item_route(
    ...
    item = await update_item(db, item_id=item_id, user_id=x_user_id, data=data)
    return ItemResponse.model_validate(item)
...
async def delete_item_route(
    ...
    await delete_item(db, item_id=item_id, user_id=x_user_id)
